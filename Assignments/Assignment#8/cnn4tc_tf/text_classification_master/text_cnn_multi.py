...
class TextCNN(object):
    ...
    def __init__(self, config):
        ...
        self.is_multichannel = config['is_multichannel']
        ...
        with tf.device('/gpu:0'), tf.name_scope("embedding"):
            self.W = tf.Variable(
                tf.random_uniform([self.vocab_size, self.embedding_size], -1.0, 1.0), trainable=is_trainable,
            ...
            self.embedded_chars_expanded.append(tf.expand_dims(self.embedded_chars, -1)) # batch, max_length, embed_dim, 1
            if self.is_multichannel:
                self.embedded_chars = tf.nn.embedding_lookup(self.W2, self.input_x)
                self.embedded_chars_expanded.append(tf.expand_dims(self.embedded_chars, -1))

        # Create a convolution + maxpool layer for each filter size
        pooled_outputs = []
        for channel in range(self.is_multichannel + 1):
            for i, filter_size in enumerate(self.filter_sizes): #filter size 별로 convolution 수행
                with tf.name_scope("conv-maxpool-%s" % filter_size):
                    ...
                    # convolution
                    conv = tf.nn.conv2d(
                        self.embedded_chars_expanded[channel],
                        W,
                        strides=[1, 1, 1, 1],
                        padding="VALID",
                        name="conv")
