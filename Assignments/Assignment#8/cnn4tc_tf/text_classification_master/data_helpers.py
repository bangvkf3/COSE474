...
# load_mr_data('./data/MR/rt-polarity.pos', './data/MR/rt-polarity.neg')
...
def load_trec_data(train_file):
    ...
    text_data = list(open(train_file, 'r').readlines())
    for line in text_data:
        line = line.split(':')
        class_ = clean_str(line[0])
        text = line[1].split(' ')
        text = ' '.join(text[1:])
        text = clean_str(text).replace('\n', '')
        text_classfied[class_].append(text)

    ABBR_labels = [[0, 0, 0, 0, 0, 1] for _ in text_classfied['abbr']]
    DESC_labels = [[0, 0, 0, 0, 1, 0] for _ in text_classfied['desc']]
    ENTY_labels = [[0, 0, 0, 1, 0, 0] for _ in text_classfied['enty']]
    HUM_labels = [[0, 0, 1, 0, 0, 0] for _ in text_classfied['hum']]
    LOC_labels = [[0, 1, 0, 0, 0, 0] for _ in text_classfied['loc']]
    NUM_labels = [[1, 0, 0, 0, 0, 0] for _ in text_classfied['num']]

    y = np.concatenate([ABBR_labels, DESC_labels, ENTY_labels, HUM_labels, LOC_labels, NUM_labels], 0)
    x_final = text_classfied['abbr'] + text_classfied['desc'] + text_classfied['enty'] + text_classfied['hum'] +\
              text_classfied['loc'] + text_classfied['num']
