...
from MNIST_torch.MNIST_cnn_model import *

def data_load():

    # MNIST dataset 다운로드
    train_val_dataset = dsets.MNIST(root='./dataset/', train=True, transform=transforms.ToTensor(), download=True)
    train_data, val_data = torch.utils.data.random_split(train_val_dataset, [55000, 5000])
    return train_data, val_data #training set, validation set


def imgshow(image, label): # 실제 image 확인을 위한 함수
    print('========================================')
    print("The 1st image:")
    print(image)
