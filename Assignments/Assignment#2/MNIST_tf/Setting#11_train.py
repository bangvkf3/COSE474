...
W1 = tf.get_variable("W1", shape=[784, 300], initializer=tf.contrib.layers.variance_scaling_initializer())
b1 = tf.Variable(tf.random_normal([300]))
L1 = tf.nn.relu(tf.matmul(X, W1) + b1)

W2 = tf.get_variable("W2", shape=[300, 400], initializer=tf.contrib.layers.variance_scaling_initializer())
b2 = tf.Variable(tf.random_normal([400]))
L2 = tf.nn.relu(tf.matmul(L1, W2) + b2)

W3 = tf.get_variable("W3", shape=[400, 150], initializer=tf.contrib.layers.variance_scaling_initializer())
b3 = tf.Variable(tf.random_normal([150]))
L3 = tf.nn.relu(tf.matmul(L2, W3) + b3)

W4 = tf.get_variable("W4", shape=[150, 10], initializer=tf.contrib.layers.variance_scaling_initializer())
