...
keep_prob = tf.placeholder(tf.float32, name="keep_prob")  # 살릴 확률
DECAY_LAMBDA = 0.01

W1 = tf.get_variable("W1", shape=[784, 200], initializer=tf.contrib.layers.variance_scaling_initializer())
...
W2 = tf.get_variable("W2", shape=[200, 200], initializer=tf.contrib.layers.variance_scaling_initializer())
b2 = tf.Variable(tf.random_normal([200]))
L2 = tf.nn.relu(tf.matmul(L1, W2) + b2)
L2 = tf.nn.dropout(L2, keep_prob)
weight_decay += tf.nn.l2_loss(W2)


W3 = tf.get_variable("W3", shape=[200, 200], initializer=tf.contrib.layers.variance_scaling_initializer())
b3 = tf.Variable(tf.random_normal([200]))
L3 = tf.nn.relu(tf.matmul(L2, W3) + b3)
L3 = tf.nn.dropout(L3, keep_prob)
weight_decay += tf.nn.l2_loss(W3)

W4 = tf.get_variable("W4", shape=[200, 10], initializer=tf.contrib.layers.variance_scaling_initializer())
b4 = tf.Variable(tf.random_normal([10]))
weight_decay += tf.nn.l2_loss(W4)


hypothesis = tf.nn.xw_plus_b(L3, W4, b4, name="hypothesis")
cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=hypothesis, labels=Y)) + DECAY_LAMBDA*weight_decay
